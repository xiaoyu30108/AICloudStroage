...
int get_share_picture_json_info(char *buf, char *user, char *token, char *md5, char *file_name)
{
    ...
    */
    ...
    //返回指定字符串对应的json对象
     //token
    child0 = cJSON_GetObjectItem(root, "token");
    if (NULL == child0)
    {
...
int get_cancel_picture_json_info(char *buf, char *user, char *urlmd5)
{
    ...
    //返回指定字符串对应的json对象
    // urlmd5
    child1 = cJSON_GetObjectItem(root, "urlmd5");
    if (NULL == child1)
    {
        LOG(SHAREPIC_LOG_MODULE, SHAREPICS_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
    ...
    // 用户名
    child3 = cJSON_GetObjectItem(root, "user");
    if (NULL == child3)
    {
        LOG(SHAREPIC_LOG_MODULE, SHAREPICS_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
...
int request_share_picture(char *user, char *filemd5, char *file_name)
{
    ...
    */
    ...
    char escaped_file_name[FILE_NAME_LEN * 2 + 1] = {0};
    int ret2 = 0;
    int count = 0;
    ...
    if (sql_escape_string(conn, user, escaped_user, sizeof(escaped_user)) != 0 ||
        sql_escape_string(conn, file_name, escaped_file_name, sizeof(escaped_file_name)) != 0)
    {
        ret = -1;
        goto END;
    }
    ...
    */
    sprintf(sql_cmd, "insert into share_picture_list (user, filemd5, file_name, urlmd5, `key`, pv, create_time) values ('%s', '%s', '%s', '%s', '%s', %d, '%s')",
            escaped_user, filemd5, escaped_file_name, urlmd5, key, 0, create_time);
