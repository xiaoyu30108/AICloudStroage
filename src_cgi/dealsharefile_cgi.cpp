...
int pv_file(char *md5, char *filename)
{
    ...
    if (sql_escape_string(conn, filename, escaped_filename, sizeof(escaped_filename)) != 0)
    {
        ret = -1;
        goto END;
    }
    ...
    //sql语句
    //查看该共享文件的pv字段
    sprintf(sql_cmd, "select pv from share_file_list where md5 = '%s' and file_name = '%s'", md5, escaped_filename);
    ...
    //更新该文件pv字段，+1
    sprintf(sql_cmd, "update share_file_list set pv = %d where md5 = '%s' and file_name = '%s'", pv+1, md5, escaped_filename);
    LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PROC, "sql:%s\n", sql_cmd);
...
int cancel_share_file(char *user, char *md5, char *filename)
{
    ...
    //===1、mysql记录操作
    //sql语句
    sprintf(sql_cmd, "update user_file_list set shared_status = 0 where user = '%s' and md5 = '%s' and file_name = '%s'", escaped_user, md5, escaped_filename);
...
int save_file(char *user, char *md5, char *filename)
{
    ...
    sprintf(sql_cmd, "select * from user_file_list where user = '%s' and md5 = '%s' and file_name = '%s'", escaped_user, md5, escaped_filename);
    ...
    */
    sprintf(sql_cmd, "insert into user_file_list(user, md5, create_time, file_name, shared_status, pv) values ('%s', '%s', '%s', '%s', %d, %d)", escaped_user, md5, time_str, escaped_filename, 0, 0);
    if(mysql_query(conn, sql_cmd) != 0)
    {
        LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PROC, "%s 操作失败\n", sql_cmd);
        ret = -1;
    ...
    {
        //插入记录
        sprintf(sql_cmd, " insert into user_file_count (user, count) values('%s', %d)", escaped_user, 1);
    }
    else if(ret2 == 0)
    {
        //更新用户文件数量count字段
        count = atoi(tmp);
        sprintf(sql_cmd, "update user_file_count set count = %d where user = '%s'", count+1, escaped_user);
    }
