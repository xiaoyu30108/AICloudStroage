...
int bind_existing_file_to_user(char *user, char *filename, char *md5)
{
    ...
    {
        sprintf(sql_cmd,
            "insert into user_file_count (user, count) values('%s', %d)", escaped_user, 1);
    }
    else if (ret2 == 0)
    {
        count = atoi(tmp);
        sprintf(sql_cmd,
            "update user_file_count set count = %d where user = '%s'", count + 1, escaped_user);
    }
    else
    {
        ret = -1;
...
// 存储文件信息到MySQL（复用upload_cgi.cpp的逻辑）
