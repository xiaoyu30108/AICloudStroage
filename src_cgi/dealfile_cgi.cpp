...
int get_json_info(char *buf, char *user, char *token, char *md5, char *filename)
{
    ...
    */
    ...
    //返回指定字符串对应的json对象
    //用户
    child1 = cJSON_GetObjectItem(root, "user");
    if(NULL == child1)
    {
    ...
    //文件md5码
    child2 = cJSON_GetObjectItem(root, "md5");
    if(NULL == child2)
    {
        LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
    ...
    //文件名字
    child3 = cJSON_GetObjectItem(root, "filename");
    if(NULL == child3)
    {
        LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
    ...
    //token
    child4 = cJSON_GetObjectItem(root, "token");
    if(NULL == child4)
    {
        LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
...
int share_file(char *user, char *md5, char *filename)
{
    ...
    */
    ...
    if (sql_escape_string(conn, user, escaped_user, sizeof(escaped_user)) != 0 ||
        sql_escape_string(conn, filename, escaped_filename, sizeof(escaped_filename)) != 0)
    {
        ret = -1;
        goto END;
    }
    ...
    {//===2、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
     //===3、如果mysql有记录，而redis没有记录，说明redis没有保存此文件，redis保存此文件信息后，再中断操作(redis操作)
        ...
        //查看此文件别人是否已经分享了
        sprintf(sql_cmd, "select * from share_file_list where md5 = '%s' and file_name = '%s'", md5, escaped_filename);
    ...
    //sql语句, 更新共享标志字段
    sprintf(sql_cmd, "update user_file_list set shared_status = 1 where user = '%s' and md5 = '%s' and file_name = '%s'", escaped_user, md5, escaped_filename);
    ...
    */
    sprintf(sql_cmd, "insert into share_file_list (user, md5, create_time, file_name, pv) values ('%s', '%s', '%s', '%s', %d)", escaped_user, md5, create_time, escaped_filename, 0);
    if (mysql_query(conn, sql_cmd) != 0)
    {
        LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败: %s\n", sql_cmd, mysql_error(conn));
        ret = -1;
...
int del_file(char *user, char *md5, char *filename)
{
    ...
    */
    ...
    {//===2、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
        ...
        //sql语句
        //查看该文件是否已经分享了
        sprintf(sql_cmd, "select shared_status from user_file_list where user = '%s' and md5 = '%s' and file_name = '%s'", escaped_user, md5, escaped_filename);
    ...
    if(share == 1)
    {
        ...
        //删除在共享列表的数据
        sprintf(sql_cmd, "delete from share_file_list where user = '%s' and md5 = '%s' and file_name = '%s'", escaped_user, md5, escaped_filename);
    ...
    //用户文件数量-1
    //查询用户文件数量
    sprintf(sql_cmd, "select count from user_file_count where user = '%s'", escaped_user);
    ret2 = process_result_one(conn, sql_cmd, tmp); //执行sql语句
...
*/
	...
	{
		count -= 1;
		sprintf(sql_cmd, "update user_file_count set count = %d where user = '%s'", count, escaped_user);
 		LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC,"%s %d, sql_cmd:%s\n", __FUNCTION__, __LINE__, sql_cmd);
    	if (mysql_query(conn, sql_cmd) != 0)
    	{
        	LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败: %s\n", sql_cmd, mysql_error(conn));
    ...
    //删除用户文件列表数据
    sprintf(sql_cmd, "delete from user_file_list where user = '%s' and md5 = '%s' and file_name = '%s'", escaped_user, md5, escaped_filename);
    ...
    sprintf(sql_cmd, "delete from user_file_ai_desc where user = '%s' and md5 = '%s'", escaped_user, md5);
    if (mysql_query(conn, sql_cmd) != 0)
    {
        LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败: %s\n", sql_cmd, mysql_error(conn));
        ret = -1;
...
int pv_file(char *user, char *md5, char *filename)
{
    ...
    //sql语句
    //查看该文件的pv字段和共享状态
    sprintf(sql_cmd, "select pv, shared_status from user_file_list where user = '%s' and md5 = '%s' and file_name = '%s'", escaped_user, md5, escaped_filename);
    ...
    sprintf(sql_cmd, "update user_file_list set pv = %d where user = '%s' and md5 = '%s' and file_name = '%s'", pv+1, escaped_user, md5, escaped_filename);
    ...
    if (shared_status == 1)
    {
        sprintf(sql_cmd, "update share_file_list set pv = %d where user = '%s' and md5 = '%s' and file_name = '%s'", pv+1, escaped_user, md5, escaped_filename);
        if (mysql_query(conn, sql_cmd) != 0)
        {
            LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败: %s\n", sql_cmd, mysql_error(conn));
            ret = -1;
