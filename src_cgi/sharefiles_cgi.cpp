...
int get_fileslist_json_info(char *buf, int *p_start, int *p_count)
{
    ...
    */
    ...
    //文件起点
    child2 = cJSON_GetObjectItem(root, "start");
    if (NULL == child2)
    {
    ...
    *p_start = child2->valueint;
    ...
    //文件请求个数
    child3 = cJSON_GetObjectItem(root, "count");
    if (NULL == child3)
    {
        LOG(SHAREFILES_LOG_MODULE, SHAREFILES_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
...
int get_ranking_filelist(int start, int count)
{
    ...
    */
    ...
    //===2、redis共享文件数量
    redis_num = rop_zset_zcard(redis_conn, FILE_PUBLIC_ZSET);
    if (redis_num == -1)
    {
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        char escaped_filename[1024 * 2 + 1] = {0};
        if (sql_escape_string(conn, filename, escaped_filename, sizeof(escaped_filename)) != 0)
        {
            LOG(SHAREFILES_LOG_MODULE, SHAREFILES_LOG_PROC, "escape filename failed\n");
            ret = -1;
            goto END;
        }
        sprintf(sql_cmd2, "select share_file_list.user, file_info.url, file_info.size, file_info.type, share_file_list.create_time from file_info, share_file_list where file_info.md5 = share_file_list.md5 and share_file_list.md5 = '%s' and share_file_list.file_name = '%s' limit 1", md5, escaped_filename);
