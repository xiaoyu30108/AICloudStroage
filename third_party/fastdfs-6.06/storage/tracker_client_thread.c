...
static int storage_calc_cpu_usage_permille()
{
	...
	idle_delta = (idle + iowait) - last_idle;
	last_total = total;
	last_idle = idle + iowait;
	...
	usage = (int)(((total_delta - idle_delta) * 1000) / total_delta);
	if (usage < 0)
	{
		return 0;
	}
...
static int storage_calc_memory_usage_permille()
{
	...
	while (fscanf(fp, "%63s %llu %31s", name, &value, unit) == 3)
	{
	...
	return (int)(((mem_total - mem_available) * 1000) / mem_total);
}
        ...
        bool *bServerPortChanged)
{
	...
	if (1)
	{
		...
		g_storage_stat.cpu_usage_permille =
			storage_calc_cpu_usage_permille();
		g_storage_stat.memory_usage_permille =
			storage_calc_memory_usage_permille();
		...
		int2buff(g_storage_stat.cpu_usage_permille, \
			pStatBuff->sz_cpu_usage_permille);
		int2buff(g_storage_stat.memory_usage_permille, \
