...
static FDFSGroupInfo *tracker_find_lowest_load_group();
	...
	*(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
			...
			ppServer++)
	{
		...
		int2buff(pStorageStat->connection.current_count, \
			...
			int2buff(pStorageStat->cpu_usage_permille, \
					pStatBuff->sz_cpu_usage_permille);
			int2buff(pStorageStat->memory_usage_permille, \
					pStatBuff->sz_memory_usage_permille);
...
static FDFSGroupInfo *tracker_find_lowest_load_group()
{
#define FDFS_DYNAMIC_LOAD_DISK_WEIGHT 0.30
#define FDFS_DYNAMIC_LOAD_CONN_WEIGHT 0.40
#define FDFS_DYNAMIC_LOAD_CPU_WEIGHT  0.20
#define FDFS_DYNAMIC_LOAD_MEM_WEIGHT  0.10
#define FDFS_DYNAMIC_LOAD_STAT_EXPIRE_SECONDS 180

	FDFSGroupInfo **ppGroup;
	FDFSGroupInfo **ppGroupEnd;
	FDFSGroupInfo *pBestGroup;
	double best_score;
	...
	for (ppGroup=g_groups.sorted_groups; ppGroup<ppGroupEnd; ppGroup++)
	{
		FDFSStorageDetail **ppServer;
		FDFSStorageDetail **ppServerEnd;
		double disk_usage_sum;
		double conn_usage_sum;
				...
				ppServer++)
		{
			...
			{
				disk_usage = 1.00 - ((double)pStorage->free_mb /
						(double)pStorage->total_mb);
				if (disk_usage < 0.00)
				{
					disk_usage = 0.00;
				}
				else if (disk_usage > 1.00)
				{
					disk_usage = 1.00;
				}
			}
			...
			if (pStorage->stat.last_heart_beat_time > 0 &&
				g_current_time - pStorage->stat.last_heart_beat_time <=
					FDFS_DYNAMIC_LOAD_STAT_EXPIRE_SECONDS &&
				pStorage->stat.connection.max_count > 0)
			{
				conn_usage = (double)pStorage->stat.connection.current_count /
						(double)pStorage->stat.connection.max_count;
				if (conn_usage < 0.00)
				{
					conn_usage = 0.00;
				}
				else if (conn_usage > 1.00)
				{
					conn_usage = 1.00;
				}
			}
			...
			if (pStorage->stat.last_heart_beat_time > 0 &&
				g_current_time - pStorage->stat.last_heart_beat_time <=
					FDFS_DYNAMIC_LOAD_STAT_EXPIRE_SECONDS)
			{
				cpu_usage = (double)pStorage->stat.cpu_usage_permille /
						1000.00;
				memory_usage = (double)pStorage->stat.memory_usage_permille /
						1000.00;
			}
			else
			{
			...
			disk_usage_sum += disk_usage;
			conn_usage_sum += conn_usage;
		...
		score = FDFS_DYNAMIC_LOAD_DISK_WEIGHT *
				(disk_usage_sum / (double)stat_count) +
			FDFS_DYNAMIC_LOAD_CONN_WEIGHT *
				(conn_usage_sum / (double)stat_count) +
			FDFS_DYNAMIC_LOAD_CPU_WEIGHT *
				(cpu_usage_sum / (double)stat_count) +
			FDFS_DYNAMIC_LOAD_MEM_WEIGHT *
				(memory_usage_sum / (double)stat_count);

		if (pBestGroup == NULL || score < best_score)
		{
			pBestGroup = *ppGroup;
			best_score = score;
		}
		...
		struct fast_task_info *pTask, char cmd)
{
	 ...
	 || cmd == TRACKER_PROTO_CMD_SERVICE_QUERY_STORE_WITH_GROUP_ALL)
	{
		...
		else if (g_groups.store_lookup == FDFS_STORE_LOOKUP_DYNAMIC_LOAD)
		{
			pStoreGroup = tracker_find_lowest_load_group();
			if (pStoreGroup == NULL)
			{
				pTask->length = sizeof(TrackerHeader);
				return ENOSPC;
...
static int tracker_deal_storage_beat(struct fast_task_info *pTask)
{
	...
	do 
	{
		...
		pStat->connection.current_count = \
			...
			pStat->cpu_usage_permille = \
				buff2int(pStatBuff->sz_cpu_usage_permille);
			pStat->memory_usage_permille = \
				buff2int(pStatBuff->sz_memory_usage_permille);
